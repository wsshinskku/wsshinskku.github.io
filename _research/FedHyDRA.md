...
The implementation covers distribution summaries, hybrid relations, graph embeddings, soft grouping, and the federated training loop. It includes experiment configurations, ablations, checkpointing, and evaluation utilities. The repository is currently private, so its source code is available only to authorized collaborators.
...
일부 클라이언트만 참여하는 라운드에서도 관계 정보를 활용하는 것이 핵심입니다. 코드 저장소는 현재 비공개이며, 접근 권한이 있는 협업자만 열람할 수 있습니다.
